...
MCRGreedySolver_t::MCRGreedySolver_t(Graph_t &g, int start, std::vector<int> goalSet)
{
	...
	m_open.push(new MCRGNode_t(m_start, m_G[m_start], {}, m_smallestCardinality[m_start], nullptr));
...
void MCRGreedySolver_t::MCRGreedy_search(Graph_t &g)
{
	...
	while (!m_open.empty())
	{
		...
		for (auto const &neighbor : neighbors)
		{
			...
			// If the neighbor has a smaller labels cardinality, update the smallest cardinality
			...
			if (labelsSize < m_smallestCardinality[neighbor])
			{
				...
				m_open.push(new MCRGNode_t(neighbor, m_G[neighbor], neighborLabels, labelsSize, current));
				continue;
			}
			if (labelsSize == m_smallestCardinality[neighbor])
			{
...
void MCRGreedySolver_t::print_survivability()
{
...
void MCRGreedySolver_t::computeLabelsAndSurvival(Graph_t &g)
{
